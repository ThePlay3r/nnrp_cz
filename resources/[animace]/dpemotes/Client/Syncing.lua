...
AddEventHandler("SyncPlayEmote", function(emote, player)
    ...
    if (DP.Shared[emote]) and (DP.Shared[emote].AnimationOptions) then
        local SyncHeading = DP.Shared[emote].AnimationOptions.SyncHeading
        if SyncHeading then
            SetEntityHeading(PlayerPedId(), SyncHeading)
        end
        if DP.Shared[emote].AnimationOptions.EmoteFreeze then
...
AddEventHandler("SyncPlayEmoteSource", function(emote, player)
    ...
    local syncHeading = heading - 180.1

    if (DP.Shared[emote]) and (DP.Shared[emote].AnimationOptions) then
        local SyncHead = DP.Shared[emote].AnimationOptions.SyncHeading
        if SyncHead then
            syncHeading = SyncHead
        end
    end
