...
window.onload = function () {
	...
	var eventCallback = {
		...
		fbanka: function() {
			if (document.getElementById('society-banka').classList.contains('shown') == false) {
				document.getElementById("society-banka").classList.add("shown")
			} else {
				document.getElementById("society-banka").classList.remove("shown")
			}
		},
		...
		setStatus: function (data) {
			...
			if (data.isdead == true) {
				...
				for (i = 0; i < data.status.length; i++) {
					// if (data.status[i].name === 'hunger' || data.status[i].name === 'thirst' || data.status[i].name == 'health') {
					// 	var statValue = Math.floor(data.status[i].value)
					// 	if (statValue < 6 && statValue > 0) {
					// 		document.getElementById('overlay-' + data.status[i].name).style.display == 'block'
					// 	} else {
					// 		document.getElementById('overlay-' + data.status[i].name).style.display == 'none'
					// 	}
					// }
					...
					if (data.status[i].name == 'armor') {
						if (statusValue === 0) {
							document.querySelector('#' + data.status[i].name).classList.add('hidden');
						} else {
							document.querySelector('#' + data.status[i].name).classList.remove('hidden');
						}
					}
				}
		...
		updateVehicle: function (data) {
			...
			var onlyFuelVehicles = [15];

			if (data.status == true) {
				if (data.type === 8) {
					vehicleSeatbelt.style.opacity = 0;
				} else {
					vehicleSeatbelt.style.opacity = 1;
				}
				...
				if (vehicleInfo.classList.contains('inactive')) {
					...
					if (vehiclesCars.indexOf(data.type) > -1) {
						...
						document.querySelector('#vehicle-gear').style.display = 'block';
					} else if (onlyFuelVehicles.includes(data.type)) {
						document.querySelector('#vehicle-others').style.display = 'none';
						document.querySelector('#vehicle-gear').style.display = 'block';
				...
				if (data.isElectric) {
					vehicleFuel.classList.add('electricCar')
				} else {
					vehicleFuel.classList.remove('electricCar')
				}
