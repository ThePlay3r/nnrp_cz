...
local cards = {
	{'As Kier',11,'https://i.imgur.com/SYKgIs6.png'},       					{'As Trefl',11,'http://167.99.90.247/testowy/img/ace_of_clubs.png'},
	{'As Karo',11,'https://i.imgur.com/9F9MYlO.png'},     						{'As Pik',11,'http://167.99.90.247/testowy/img/ace_of_spades.png'},
	{'Walet Pik',10,'https://i.imgur.com/o33iBQo.png'},    					    {'Dziesiątka Pik',10,'http://167.99.90.247/testowy/img/10_of_spades.png'},  
	{'Dziewiątka Pik',9,'https://i.imgur.com/2TlWizu.png'},   					{'Ósemka Pik',8,'http://167.99.90.247/testowy/img/8_of_spades.png'},        
	{'Siódemka Pik',7,'https://i.imgur.com/oOsAiLN.png'},     				    {'Szóstka Pik',6,'http://167.99.90.247/testowy/img/6_of_spades.png'},
	{'Piątka Pik',5,'https://i.imgur.com/dBIIXUD.png'},      					{'Czwórka Pik',4,'http://167.99.90.247/testowy/img/4_of_spades.png'},     
	{'Trójka Pik',3,'https://i.imgur.com/dss9VVS.png'},      					{'Dwójka Pik',2,'http://167.99.90.247/testowy/img/2_of_hearts.png'},      
	{'Dama Kier',10,'https://i.imgur.com/cdrw0sR.png'},    						{'Walet Kier',10,'http://167.99.90.247/testowy/img/jack_of_hearts.png'},  
...
AddEventHandler('program-casino:hasEnteredMarker', function(zone)
	if zone =='Croupier' then
		...
		CurrentActionMsg  = "~r~[E] ~b~pro otevreni stolu"
		CurrentActionData = {}
	elseif zone == "Player1" then
		CurrentAction     = 'Player1'
		CurrentActionMsg  = "~r~[E] ~b~pro pripojeni jako prvni hrac"
		CurrentActionData = {}
	elseif zone == "Player2" then
		CurrentAction     = 'Player2'
		CurrentActionMsg  = "~r~[E] ~b~pro pripojeni jako druhy hrac"
		CurrentActionData = {}
	end
end)
...
AddEventHandler('program-casino:notifyPlayers', function(quantity)
	...
	if GetDistanceBetweenCoords(coords, Config.BlackjackSites.Croupier.Pos.x, Config.BlackjackSites.Croupier.Pos.y, Config.BlackjackSites.Croupier.Pos.z, true) < 29 then
		ESX.ShowAdvancedNotification("~p~BlackJack", "~p~Stawka: ~g~"..quantity.."$", "~p~stul se otevrel. Hraci s GOLDEN/PLATINUM tiketem maji prednost. ", "CHAR_BANK_BOL", 9)
	end
	Citizen.Wait(5000)
end)
...
AddEventHandler('program-blackjack:givecards', function()
	...
	local finish = false
	ESX.ShowNotification("~p~Pripojil ses ke stolu tvoje prvni karta je ~b~" ..cards[card][1].. " ~p~o hodnoty ~b~"..cards[card][2].."~n~~b~" ..wynik.." ~p~pkt")
	ESX.ShowNotification("~o~Pro dalsi kartu stiskni ~r~ [F]~o~ pokud ti karty staci stiskni ~r~[X]")
	...
	while not finish do
		...
		if IsControlJustReleased(0, Keys["F"]) then
			...
			ESX.ShowNotification("~p~Zvolena karta je ~b~" ..cards[card][1].. " ~p~o hodnoty ~b~"..cards[card][2].."~n~~p~dohromady mas  ~b~" ..wynik.." ~p~pkt")
			if wynik>21 then
				if aces > 0 then
					aces = aces-1
					wynik = wynik - 10
					ESX.ShowNotification("~p~padlo ti ESO ktere se zapocitalo jako 1 misto 11")
				else
					finish=true
...
function Croupier()
		...
		ESX.UI.Menu.Open(
			...
			function(data2, menu)
				...
				if quantity == nil or quantity < 1 then
					ESX.ShowNotification("Neplatna hodnota")
				else
					menu.close()
	...
	elseif PlayerData.job.name ~= 'casino' then
		ESX.ShowNotification("Musis byt krupier aby jsi mohl otevrit stul")
	elseif gameInProgress == true and PlayerData.job.name == 'casino' then
		ESX.ShowNotification("~b~Aktualne je otevrena hra pokud ji chces ukoncit stiskni~r~[X]~b~ ")
	end
...
function Player1()
	ESX.TriggerServerCallback('program-blackjack:checkStarting', function(result)
		if not result then
			ESX.ShowNotification('~r~Stul je uzavreny popros krupiera aby stul otevrel!')
		else
			if isPlatinium then
				local elements = {
					{label = "Hraj za spinave penize",  value = 'black'},
					{label = "Hraj za ciste penize",  value = 'money'}
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'wybierz_walute', {
					...
					elseif data.current.value == 'black' then
						ESX.TriggerServerCallback('program-blackjack:checkBlackMoney', function(result2)
							if not result2 then
								ESX.ShowNotification('~r~Nemate dost penez na pripojeni do hry!')
							else
								isP1playing = true
