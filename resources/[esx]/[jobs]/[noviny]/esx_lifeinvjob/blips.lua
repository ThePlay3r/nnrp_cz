...
AddEventHandler('esx_farmajob:handleBlips', function(jobName)
	deleteBlips()
	createBlips(jobName)
end)

function createBlip (blipMarker, jobName)
	if blipMarker.IsPublic == false and jobName ~= 'lifeinvader' then
		return
    end
...
function createBlips(jobName)
	createBlip(Config.Blips.LifeInvader, jobName)
end
