...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	TriggerEvent('esx_kosteljob:handleBlips', jobName)
end)
...
AddEventHandler('esx_kosteljob:hasEnteredMarker', function(zone, data)
	...
	if zone == 'Transform' then
		...
		CurrentActionMsg  = 'Stiskni ~INPUT_CONTEXT~ pro vyrobu ruzencu'
		CurrentActionData = {}
	end

	if zone == 'Products' then
		CurrentAction     = 'menu_farming'
		CurrentActionMsg  = 'Stiskni ~INPUT_CONTEXT~ pro sber krizku'
		CurrentActionData = {data = data}
	end
end)

AddEventHandler('esx_kosteljob:hasExitedMarker', function(zone)
	CurrentAction = nil
	IsFarming = false
	IsTransforming = false
	TriggerServerEvent('esx_kosteljob:stopFarming')
	TriggerServerEvent('esx_kosteljob:stopTransform')
	...
	while true do
		...
		if jobName == 'kostel' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_kosteljob:hasEnteredMarker', currentZone, currentIndex)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_kosteljob:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		function(data, menu)
			...
			if player ~= -1 and distance <= 3.0 then
				...
				if amount == nil then
					...
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_kostel', _U('billing'), amount)
				end
			else
...
function openSocietyActionsMenu ()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'kostel_actions',
		{
			title    = _U('kostel'),
			align    = 'left',
			elements = elements
...
function openCloakroomMenu ()
	local elements = {
		...
		{value = 'kostel_outfit', label = 'Pracovni obleceni'},
	}
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0,  Config.Keys['E']) and jobName == 'kostel' then
				if CurrentAction == 'menu_cloakroom' then
					openCloakroomMenu()
				end

				if CurrentAction == 'menu_vault' then
					TriggerEvent('esx_kosteljob:openVaultMenu')
				end

				if CurrentAction == 'sell_products' then
					TriggerServerEvent('esx_kosteljob:startSell')
				end
				...
				if CurrentAction == 'menu_transform' then
					IsTransforming = true
					TriggerServerEvent('esx_kosteljob:startTransform')
				end

				if CurrentAction == 'menu_farming' then
					IsFarming = true
					TriggerServerEvent('esx_kosteljob:startFarming', CurrentActionData.data)
				end

				if CurrentAction == 'menu_boss_actions' and (jobGrade == 'boss' or jobGrade == 'lowerboss1' or jobGrade == 'lowerboss2') then
					ESX.UI.Menu.CloseAll()
					local options = {}
					TriggerEvent('esx_society:openBossMenu', 'kostel', function(data, menu)
						menu.close()
						CurrentAction     = 'menu_boss_actions'
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
		...
		if IsControlJustReleased(0, Config.Keys['F6']) and jobName == 'kostel' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'kostel_actions') then
