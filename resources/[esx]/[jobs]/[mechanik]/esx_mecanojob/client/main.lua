...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		--TriggerServerEvent('esx_mecanojob:onNPCJobCILovePizzaompleted')
	end
end
...
function OpenMecanoActionsMenu()
	...
	local elements = {
		-- nove se o to stara vehicleSpawner
...
function OpenMobileMecanoActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mecano_actions', {
		...
		elements = {
			...
			{label = 'Ovladani dilny', value = 'dilna'},
		}
	}, function(data, menu)
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				exports['progressBars']:startUI(10000, "Odemykání")
				Citizen.Wait(10000)
	...
	elseif data.current.value == 'fix_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				exports['progressBars']:startUI(20000, "Opravování vozidla")
				Citizen.Wait(20000)
	...
	elseif data.current.value == 'dilna' then
		local elements = {
			{label = 'Vrata lakovny', value = 'v_ilev_spraydoor'},
			-- {label = 'Zvedak', value = 'lift'},
		}

		local playerPed = PlayerPedId()
		local coords    = GetEntityCoords(playerPed)

		if GetDistanceBetweenCoords(coords, -212.33, -1320.86, 30.86, true) > 30 then
			elements = {value = 'none', label = 'Pro ovladani musis byt v dilne'}
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mecano_actions_spawn', {
			...
			if data2.current.value == 'v_ilev_spraydoor' then
				local x = -202.3631
				local y = -1324.331
				local z = 31.40135
				local object = GetClosestObjectOfType(x, y, z, 3.0, 'v_ilev_spraydoor', false, false, false)

				if DoesEntityExist(object) then
					DeleteEntity(object)
				else
					local model   = data2.current.value
					local heading = 91.0472259
			...
			if data2.current.value == 'lift' then
				...
				local y = -1322.573
				local z = 30.13525
				local object = GetClosestObjectOfType(x, y, z, 3.0, 'lift', false, false, false)

				if DoesEntityExist(object) then
					DeleteEntity(object)
				else
					local model   = data2.current.value
					local heading = 358.689972
					ESX.Game.SpawnObject(model, {
						x = x,
						y = y,
						z = z
	...
	elseif data.current.value == 'del_vehicle' then
		...
		local playerPed = PlayerPedId()
		local text = "*Volá pepu na odtah vozidla*"
	...
	elseif data.current.value == 'object_spawner' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mecano_actions_spawn', {
			...
			align    = 'left',
			elements = {
				{label = _U('roadcone'), value = 'prop_carjack'},
				{label = _U('kuzel'), value = 'prop_mp_cone_02'},
				{label = _U('pneu'), value = 'imp_prop_impexp_tyre_01a'},
				{label = _U('paleta'), value = 'prop_pallet_01a'},
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleILovePizzaInGarage', 'mecano', vehicleProps)
