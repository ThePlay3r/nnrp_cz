...
local function Harvest2(source, timeHash)
	SetTimeout(4000, function()

		if PlayersHarvesting2[source] == timeHash then
			local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('esx_mecanojob:startHarvest2', function()
	...
	if PlayersHarvesting2[_source] == nil then
		local timeHash = os.time()
		PlayersHarvesting2[_source] = timeHash
		TriggerClientEvent('esx:showNotification', _source, _U('recovery_repair_tools'))
		Harvest2(_source, timeHash)
	end
end)
...
AddEventHandler('esx_mecanojob:stopHarvest2', function()
	local _source = source
	if PlayersHarvesting2[_source] ~= nil then
		PlayersHarvesting2[_source] = nil
	end
end)

local function Harvest3(source, timeHash)
	SetTimeout(4000, function()

		if PlayersHarvesting3[source] == timeHash then
			local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('esx_mecanojob:startHarvest3', function()
	...
	if PlayersHarvesting3[_source] == nil then
		local timeHash = os.time()
		PlayersHarvesting3[_source] = timeHash
		TriggerClientEvent('esx:showNotification', _source, _U('recovery_body_tools'))
		Harvest3(_source, timeHash)
	end
end)
...
AddEventHandler('esx_mecanojob:stopHarvest3', function()
	local _source = source
	if PlayersHarvesting3[_source] ~= nil then
		PlayersHarvesting3[_source] = nil
	end
end)

local function Craft(source, timeHash)
	SetTimeout(4000, function()

		if PlayersCrafting[source] == timeHash then
			local xPlayer = ESX.GetPlayerFromId(source)
			local GazBottleQuantity = xPlayer.getInventoryItem('gazbottle').count
...
AddEventHandler('esx_mecanojob:startCraft', function()
	...
	if PlayersCrafting[_source] == nil then
		local timeHash = os.time()
		PlayersCrafting[_source] = timeHash
		TriggerClientEvent('esx:showNotification', _source, _U('assembling_blowtorch'))
		Craft(_source, timeHash)
	end
end)
...
AddEventHandler('esx_mecanojob:stopCraft', function()
	local _source = source
	if PlayersCrafting[_source] ~= nil then
		PlayersCrafting[_source] = nil
	end
end)

local function Craft2(source, timeHash)
	SetTimeout(4000, function()

		if PlayersCrafting2[source] == timeHash then
			local xPlayer = ESX.GetPlayerFromId(source)
			local FixToolQuantity = xPlayer.getInventoryItem('fixtool').count
...
AddEventHandler('esx_mecanojob:startCraft2', function()
	...
	if PlayersCrafting2[_source] == nil then
		local timeHash = os.time()
		PlayersCrafting2[_source] = timeHash
		TriggerClientEvent('esx:showNotification', _source, _U('assembling_repair_kit'))
		Craft2(_source, timeHash)
	end
end)
...
AddEventHandler('esx_mecanojob:stopCraft2', function()
	local _source = source
	if PlayersCrafting2[_source] ~= nil then
		PlayersCrafting2[_source] = nil
	end
end)

local function Craft3(source, timeHash)
	SetTimeout(4000, function()

		if PlayersCrafting3[source] == timeHash then
			local xPlayer = ESX.GetPlayerFromId(source)
			local CaroToolQuantity = xPlayer.getInventoryItem('carotool').count
...
AddEventHandler('esx_mecanojob:startCraft3', function()
	...
	if PlayersCrafting3[_source] == nil then
		local timeHash = os.time()
		PlayersCrafting3[_source] = timeHash
		TriggerClientEvent('esx:showNotification', _source, _U('assembling_body_kit'))
		Craft3(_source, timeHash)
	end
end)
