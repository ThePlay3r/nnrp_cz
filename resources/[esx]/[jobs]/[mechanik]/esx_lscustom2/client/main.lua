				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					if isRimMod then
						...
						if Config.LogHook then
							TriggerServerEvent('esx_lscustom2:addInLog', GetPlayerName(PlayerId()), data.current.modType, price, vehicleData.plate, vehicleName)
						end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'mecano') or (PlayerData.job ~= nil and PlayerData.job.name == 'mecano') or Config.IsmechanicJobOnly == false then
				for k,v in pairs(Config.Zones) do
