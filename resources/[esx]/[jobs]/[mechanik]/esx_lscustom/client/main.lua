		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					local vehicleName = exports.nnvozidla:getVehicleName(GetDisplayNameFromVehicleModel(vehModel))
					...
					if isRimMod then
						...
						if string.match(data.current.label, _U('by_default')) then
							price = 0
						end
						TriggerServerEvent("esx_lscustom:buyMod", price)
						if Config.LogHook then
							TriggerServerEvent('esx_lscustom:addInLog', GetPlayerName(PlayerId()), data.current.modType, price, vehicleData.plate, vehicleName)
						end
...
function UpdateMods(data)
	...
	if data.modType ~= nil then
		...
		elseif data.modType == 'modXenon' then
			props['modXenon'] = true
			props['modXenonColor'] = data.modNum
			ESX.Game.SetVehicleProperties(vehicle, props)
			props = {}
	...
	for k,v in pairs(Config.Menus) do
		...
		if data.value == k then
			...
			if v.modType ~= nil then
				...
				elseif v.modType == 22 then -- NEON
					...
					for i=1, #colors, 1 do
						table.insert(elements, {
							label = '<span>' .. colors[i].label .. ' - <span style="color:green;">$' .. price .. '</span>',
							modType = k,
							modNum = colors[i].color
						})
					end
				...
				elseif v.modType == 'color1' or v.modType == 'color2' or v.modType == 'pearlescentColor' or v.modType == 'wheelColor' or v.modType == 'modDashboardColor' or v.modType == 'modInteriorColor' then -- RESPRAYS
				...
				elseif v.modType == 17 then -- TURBO
				 ...
				 --LIVERY FIX by DefaultLuca
			elseif v.modType == 48 then -- Livery
				local _label = ''
				--[edit] local modCount = GetVehicleLiveryCount(vehicle)
      ...
      --END LIVERY FIX by DefaultLuca
				...
				if data.value == 'primaryRespray' or data.value == 'secondaryRespray' or data.value == 'pearlescentRespray' or data.value == 'modFrontWheelsColor' or
				data.value == 'dashboardColor' or data.value == 'interiorColor' then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'mechanic') or (PlayerData.job ~= nil and PlayerData.job.name == 'mecano') or Config.IsmechanicJobOnly == false then
				for k,v in pairs(Config.Zones) do
