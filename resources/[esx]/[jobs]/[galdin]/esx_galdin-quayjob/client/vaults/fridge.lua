...
AddEventHandler('esx_galdinjob:openFridgeMenu', function()
	local elements = {
		{label = _U('get_object'), value = 'get_stock'},
		{label = _U('put_object'), value = 'put_stock'},
		{label = 'Koupit pochutiny', value = 'buy_pochutina'}
	}
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			if data.current.value == 'buy_pochutina' then
				TriggerEvent('esx_galdinjob:openShopMenu', 'Pochutiny')
			end
...
function OpenGetFridgeStocksMenu()
	...
	ESX.TriggerServerCallback('esx_galdinjob:getFridgeStockItems', function(items)
		local elements = {}
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil then
							...
							menu2.close()
							TriggerServerEvent('esx_galdinjob:getFridgeStockItem', itemName, count)
...
function OpenPutFridgeStocksMenu()
    ESX.TriggerServerCallback('esx_galdinjob:getPlayerInventory', function(inventory)
		local elements = {}
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil then
							...
							TriggerServerEvent('esx_galdinjob:putFridgeStockItems', itemName, count)
