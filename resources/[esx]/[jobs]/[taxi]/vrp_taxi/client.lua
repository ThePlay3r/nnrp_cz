...
local costPerMile = 800.0
local initialFare = 800.0 -- the cost to start a fare
    ...
    while true do
      ...
      if(IsInTaxi()) then
        if (inTaxi == false) then
          msg = "Stiskni HOME pro zobrazeni taximetru"
...
RegisterCommand('zapnoutMetr', function()
  TriggerEvent('taxi:toggleHire')
end)
RegisterCommand('resetMetru', function()
  TriggerEvent('taxi:resetMeter')
end)

RegisterKeyMapping('zapnoutMetr', 'Zapnout taximetr', 'keyboard', '')
RegisterKeyMapping('resetMetru', 'Reset taximetru', 'keyboard', '')
...
AddEventHandler('taxi:updatefare', function(veh)
  ...
  local _fare = DecorGetFloat(veh, "fares")
  local _miles = DecorGetFloat(veh, "miles")
  local _kilometres = _miles * 1.609344
  local farecost = _fare + (_miles * DecorGetFloat(veh, "costPerMile"))
    ...
    player = string.format("%.2f", _kilometres),
