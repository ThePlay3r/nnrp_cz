...
AddEventHandler("esx-qalle-jail:openJailMenu", function()
	TriggerServerEvent("nnchrp:kirokuSuru", "se pokusil otevrit jail menu")
end)

RegisterNetEvent("esx-qalle-jail:jailPILovePizzalayer")
AddEventHandler("esx-qalle-jail:jailPILovePizzalayer", function(newJailTime)
	jailTime = newJailTime
...
function JailLogin()
	...
	ESX.ShowNotification("Usnul/a si ve veznici, a proto si byl/a znovu prenesen/a do cely!")
...
function UnJail()
	...
	ESX.ShowNotification("Byl/a jsi propusten/a! Hodne stesti!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Zbyva ti " .. jailTime .. " minut ve vezeni!")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Balit"

					if not v["state"] then
						PackageText = "Jiz zabaleno"
					end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Tento balicek jsi jiz bral/a!")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Otevrit dvere", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Probiha baleni... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Polozit balicek", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Uveznit nejblizsi osobu", value = "jail_closest_player" },
				{ label = "Propustit osobu", value = "unjail_player" }
			}
		}, 
	function(data, menu)
		...
		if action == "jail_closest_player" then
			...
			ESX.UI.Menu.Open(
          		...
          		{
            		title = "Doba uvezneni (v minutach)"
          		},
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Cas musi byt v minutach!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Zadny hrac pobliz!")
					else
						ESX.UI.Menu.Open(
							'dialog', GetCurrentResourceName(), 'jail_choose_reason_menu',
							{
							  title = "Duvod uvezneni"
							},
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Musis napsat duvod uvezneni!")
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("esx-qalle-jail:jailPILovePizzalayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Veznice je prazdna!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Vezen: " .. playerArray[i].name .. " | Doba uvezneni: " .. playerArray[i].jailTime .. " minut", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Propustit osobu",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
