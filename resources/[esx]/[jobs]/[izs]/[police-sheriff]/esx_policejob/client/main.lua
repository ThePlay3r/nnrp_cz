...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			if uniform == 'bullet_wear' or uniform == 'swat_wear' then
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		-- {label = _U('bullets_wear'), uniform = 'bullets_wear'},
		-- {label = _U('gilet_wear'), uniform = 'gilet_wear'},
		{label = _U('police_wear'), uniform = grade},
		{label = 'Obleceni na motorku', uniform = 'motobike_wear'}
	}

	if grade ~= 'recruit' then
		table.insert(elements, {label = 'Zásahová výbava', uniform = 'swat_wear'})
		table.insert(elements, {label = 'Pilot', uniform = 'pilot_wear'})
		table.insert(elements, {label = 'Pilot s helmou', uniform = 'swat_pilot_wear'})
	end

	-- if grade == 'intendent' or grade == 'chef' or grade == 'boss' or grade == 'zastupce' then
		-- table.insert(elements, {label = _U('swat_wear'), uniform = 'swat_wear'})
		-- table.insert(elements, {label = 'SWAT Pilot', uniform = 'swat_pilot_wear'})
	-- end

	-- if grade == 'chef' or grade == 'boss' or grade == 'zastupce' then
	-- 	table.insert(elements, {label = 'Obleceni detektiva', uniform = 'detective_wear'})
	-- end
...
function OpenArmoryMenu(station)
	...
	if ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'lowerboss1' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Faktury', {
					...
					if amount == nil or amount < 0 then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							menu.close()
							TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_police', 'Policie', amount)
						end
					end
				end, function(data, menu)
					menu.close()
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = _U('box'), model = 'prop_barrier_work04a'},
					{label = _U('cash'), model = 'prop_barrier_work02a'},
					{label = 'Terc Cerny', model = 'gr_prop_gr_target_05a'},
					{label = 'Terc Bily', model = 'gr_prop_gr_target_05b'}
			}}, function(data2, menu2)
				local playerPed = PlayerPedId()
...
function OpenFineMenu(player)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine', {
		...
		align    = 'left',
		elements = {
			{label = 'Dopravni prestupky', value = 0},
			{label = 'Prestupky', value = 1},
			{label = 'Proti obcanum', value = 2},
			{label = 'Proti policistum', value = 3},
			{label = 'Latky', value = 4},
			{label = 'Zbrane', value = 5},
			{label = 'Ostatni', value = 6},
			{label = 'Legalizace', value = 7}
	}}, function(data, menu)
...
RegisterNetEvent('esx_policejob:haILovePizzandcuff')
AddEventHandler('esx_policejob:haILovePizzandcuff', function()
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			...
			for k,v in pairs(Config.PoliceStations) do
				...
				if Config.EnablePlayerManagement and (ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'lowerboss1') then
