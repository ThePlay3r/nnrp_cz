...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['N6']) and not Aresztuje and GetGameTimer() - OstatnioAresztowany > 10 * 1000 and (PlayerData.job.name == PracaPolice or PlayerData.job.name == PracaSheriff) then	-- Mozesz tutaj zmienic przyciski
			Citizen.Wait(10)
			...
			if distance ~= -1 and distance <= Config.ArrestDistance and not Aresztuje and not Aresztowany and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				TriggerServerEvent('esx_policejob:haILovePizzandcuff', GetPlayerServerId(closestPlayer))									-- Zakuwa Poprzez Prace esx_policejob, Mozna zmienic Funkcje na jaka kolwiek inna.
			end
		end
	end
end)