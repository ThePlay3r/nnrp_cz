...
function setUniform(uniform, playerPed)
 ...
 CreateThread(function()
    while true do 
        Citizen.Wait(0)
		if IsControlJustReleased(0, 161) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'id_card_menu') then
			openMenu()
		end
	end
end)
...
CreateThread(function()
    while true do 
        Citizen.Wait(5000)
            ...
            TriggerServerEvent('sheriffbadge:ItemsBadge', GetPlayerPed(-1))
		end	
    end
end)
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('sheriff_wear'), uniform = 'officer_wear'},
		{label = _U('bullet_wear'), uniform = 'bullet_wear'}
	}

	if grade == 'lowerboss1' or grade == 'boss' then
		table.insert(elements, {label = 'Vedeni', uniform = 'vedeni_wear'})
	end

	if ESX.PlayerData.job.grade >= 1 then
		table.insert(elements, {label = 'Senior Deputy', uniform = 'deputy_wear'})
	end

	if grade ~= 'kadet' then
		table.insert(elements, {label = 'Pilot', uniform = 'pilot_wear'})
		table.insert(elements, {label = 'Vycvikar', uniform = 'vycvikar_wear'})
		table.insert(elements, {label = 'Supervisor', uniform = 'supervisor_wear'})
	end
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Faktury', {
					...
					if amount == nil or amount < 0 then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							menu.close()
							TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_sheriff', 'Policie', amount)
						end
					end
				end, function(data, menu)
					menu.close()
