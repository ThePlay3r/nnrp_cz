...
function OpenVigneActionsMenu()
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and (PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'lowerboss1') then
		table.insert(elements, {label = _U('take_stock'), value = 'get_stock'})
	end
  
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and (PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'lowerboss1') then -- Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss'
...
function OpenMobileVigneActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_vigne_actions', {
			...
			elseif data.current.value == 'search' then
				TriggerServerEvent('esx_vigneronjob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
...
function OpenBodySearchMenu(player)
	ESX.TriggerServerCallback('esx_vigneronjob:getOtherPlayerData', function(data)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_vigneronjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
AddEventHandler('esx_vigneronjob:hasEnteredMarker', function(zone)
	if zone == 'exitMarker' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		CurrentAction     = zone
...
function blips()
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		...
		for k,v in pairs(Config.Zones)do
			if v.Type == 1 and v.Name ~= 'Garage' then
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  38) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 300 then
				...
				elseif CurrentAction == 'vehicle_spawner_menu' then
					exports.nncore:OpenVehicleSpawnerMenu(
						Config.VehicleShopCoords, Config.VehicleSpawnPoints, Config.AuthorizedVehicles
					)
				elseif CurrentAction == 'delete_vehicle' then

					if Config.EnableSocietyOwnedVehicles then
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData.vehicle)
						TriggerServerEvent('esx_society:putVehicleILovePizzaInGarage', 'vigne', vehicleProps)
					end
