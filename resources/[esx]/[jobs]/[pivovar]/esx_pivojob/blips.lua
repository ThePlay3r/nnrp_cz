...
AddEventHandler('esx_pivovarjob:handleBlips', function(jobName)
	deleteBlips()
	createBlips(jobName)
end)

function createBlip (blipMarker, jobName)
	if blipMarker.IsPublic == false and jobName ~= 'pivovar' then
		return
    end
...
function createBlips(jobName)
	createBlip(Config.Blips.Pivovar, jobName)
