...
function OpenVaultMenu()
    ...
    local elements = {
      {label = 'Vzit predmet', value = 'get_stock'},
      {label = 'Vlozit predmet', value = 'put_stock'}
    }
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        CurrentActionMsg  = "Stiskni ~g~E~w~ pro otevreni skladu"
        CurrentActionData = {}
      end
    )
  end
...
function OpenFridgeMenu()
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        CurrentActionMsg  = "Stiskni ~g~E~w~ k otevreni lednice"
        CurrentActionData = {}
      end
    )
...
function OpenCozinharMenu()
	...
	local elements = {
    {label = "Smazena vejce", value = 'ovoes'},
	  {label = "Hamburguer", value = 'ham'},
    {label = "Grilovana chobotnice", value = 'polvo'},
    {label = "Pecene teleci maso", value = 'vitela'},
	  {label = "Anglicka snidane", value = 'anglishbreak'},
	  {label = "Ryba z hranolkami", value = 'fishchips'},
	  {label = "Gulas", value = 'gulas'},
	  {label = "Humr", value = 'lobster'},
	  {label = "Nudle", value = 'nudle'},
	  {label = "Palacinky", value = 'palacinky'},
	  {label = "Pizza", value = 'pizza'},
	  {label = "Dinova polevka", value = 'pumpkinsoap'},
	  {label = "Ramen", value = 'ramen'},
	  {label = "Pecene rebirka", value = 'ribs'},
	  {label = "Rizek s hranolkami", value = 'rizekhranolky'},
	  {label = "Losos s bramborami", value = 'salmon'},
	  {label = "Steak s bramborami", value = 'seakpotato'},
	  {label = "Spagety", value = 'spagety'},
	  {label = "Spiz", value = 'spiz'},
	  {label = "Tiramisu", value = 'tiramisu'},
	  {label = "Tortelini", value = 'tortelini'},
	  {label = "Wafle", value = 'wafle'}
    }
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        CurrentActionMsg  = "Stiskni ~g~E~w~ k otevreni grilu"
        CurrentActionData = {}
      end
    )
...
function OpenSocietyActionsMenu()
  ...
  -- table.insert(elements, {label = 'Varit',    value = 'Varit'})
  ...
  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
    ...
    if amount == nil or amount < 0 then
      ...
      if closestPlayer == -1 or closestDistance > 3.0 then
        ...
        menu.close()
        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cozinheiro', 'Zensai Restaurace', amount)
      end
    end
...
AddEventHandler('esx_cozinheirojob:hasEnteredMarker', function(zone)
    ...
    if zone == 'Cloakrooms' then
      ...
      if zone == 'Vaults' and IsGradeBoss() then
        CurrentAction     = 'menu_vault'
        CurrentActionMsg  = "Stiskni ~g~E~w~ pro vstup do skladu"
        CurrentActionData = {}
      end

    if zone == 'Fridge' or zone == 'Fridge2' or zone == 'Fridge3' then
      CurrentAction     = 'menu_fridge'
...
function createBlip (BlipConfig, jobName)
  if BlipConfig.publicBlip == false and jobName ~= 'cozinheiro' then
    return
  end
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleILovePizzaInGarage', 'cozinheiro', vehicleProps)
