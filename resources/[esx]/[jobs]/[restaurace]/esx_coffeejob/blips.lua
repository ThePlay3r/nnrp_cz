...
AddEventHandler('esx_coffeejob:handleBlips', function(jobName)
	deleteBlips()
	createBlips(jobName)
end)

function createBlip (blipMarker, jobName)
	if blipMarker.IsPublic == false and jobName ~= 'coffee' then
		return
    end
...
function createBlips(jobName)
	createBlip(Config.Blips.Coffee, jobName)
	createBlip(Config.Blips.Shop, jobName)
