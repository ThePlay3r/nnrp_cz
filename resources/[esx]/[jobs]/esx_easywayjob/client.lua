...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	TriggerEvent('esx_easywayjob:handleBlips', jobName)
end)
...
AddEventHandler('esx_easywayjob:hasEnteredMarker', function(zone, data)
	...
	if zone == 'Vaults' and (jobGrade == 'boss' or jobGrade == 'lowerboss1' or jobGrade == 'barista') then
		CurrentAction     = 'menu_vault'
		CurrentActionMsg  = _U('open_vault')
		CurrentActionData = {}
	end

	if zone == 'Sell' or zone == 'Sell2' or zone == 'Sell3' then
...
AddEventHandler('esx_easywayjob:hasExitedMarker', function(zone)
	CurrentAction = nil
	IsFarming = false
	TriggerServerEvent('esx_easywayjob:stopFarming')
	...
	while true do
		...
		if jobName == 'easyway' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_easywayjob:hasEnteredMarker', currentZone, currentIndex)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_easywayjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		function(data, menu)
			...
			if player ~= -1 and distance <= 3.0 then
				...
				if amount == nil then
					...
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_easyway', _U('billing'), amount)
				end
			else
...
function openSocietyActionsMenu ()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'easyway_actions',
		{
			title    = _U('easyway'),
			align    = 'left',
			elements = elements
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0,  Config.Keys['E']) and jobName == 'easyway' then
				if CurrentAction == 'menu_cloakroom' then
					openCloakroomMenu()
				end

				if CurrentAction == 'menu_vault' then
					TriggerEvent('esx_easywayjob:openVaultMenu')
				end

				if CurrentAction == 'sell_products' then
					TriggerServerEvent('esx_easywayjob:startSell', CurrentActionData.zone)
				end
				...
				if CurrentAction == 'menu_farming' then
					IsFarming = true
					TriggerServerEvent('esx_easywayjob:startFarming', CurrentActionData.data)
				end

				if CurrentAction == 'menu_boss_actions' and (jobGrade == 'boss' or jobGrade == 'lowerboss1') then
					ESX.UI.Menu.CloseAll()
					local options = {}
					TriggerEvent('esx_society:openBossMenu', 'easyway', function(data, menu)
						menu.close()
						CurrentAction     = 'menu_boss_actions'
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
		...
		if IsControlJustReleased(0, Config.Keys['F6']) and jobName == 'easyway' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'easyway_actions') then
