...
AddEventHandler('napojak2:hasEnteredMarker', function()
    isInZone = true
end)

AddEventHandler('napojak2:hasExitedMarker', function()
    isInZone = false
    CloseNapojak()
end)
...
Citizen.CreateThread(function()
	while true do
        ...
        if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
            ...
            TriggerEvent('napojak2:hasEnteredMarker')
        end

        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('napojak2:hasExitedMarker')
        end
	end
end)
