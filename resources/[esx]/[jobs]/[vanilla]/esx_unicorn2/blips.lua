...
AddEventHandler('esx_unicorn2:handleBlips', function(jobName)
	deleteBlips()
	createBlips(jobName)
end)

function createBlip (blipMarker, jobName)
	if blipMarker.IsPublic == false and jobName ~= 'unicorn' then
		return
    end
...
function createBlips(jobName)
	-- createBlip(Config.Blips.Fotky, jobName)
	-- createBlip(Config.Blips.Casopisy, jobName)
