...
function ManageFuelUsage(vehicle)
	...
	if not DecorExistOn(vehicle, Config.FuelDecor) then
		if vehClass == 15 then
			...
			SetFuel(vehicle, math.random(500, 750) / 10)
		end
	...
	if IsVehicleEngineOn(vehicle) then
		local entityModel = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))
		local calculatedFuelUsage = Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)]

		if entityModel == 'MODELS' or entityModel == 'F PACE' or entityModel == 'teslax' then
			calculatedFuelUsage = 0.25
		end

		SetFuel(vehicle, GetVehicleFuelLevel(vehicle) - calculatedFuelUsage * (Config.Classes[GetVehicleClass(vehicle)] or 1.0) / 10)
	end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if pumpDistance < 2.5 then
			...
			pumpHash = GetEntityModel(pumpObject)
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		local multiplier = Config.CostMultiplier
		if pumpHash == Config.SuperchargerModel then
			multiplier = 1.0
		end
		local extraCost = fuelToAdd / 1.5 * multiplier
...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			if pumpHash == Config.SuperchargerModel then
				DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.CancelFuelingPumpEle .. extraString)
			else
...
function hasPlayerElectricCar (vehicle)
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				local hasElectricCar = hasPlayerElectricCar(GetVehiclePedIsIn(ped))
				
				if pumpHash == Config.SuperchargerModel and hasElectricCar then
					DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, Config.Strings.ExitVehicleEle)
				elseif not pumpHash == Config.SuperchargerModel and not hasElectricCar then
					DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, Config.Strings.ExitVehicle)
				end
			else
				local vehicle = GetPlayersLastVehicle()
				local vehicleCoords = GetEntityCoords(vehicle)
				local isElectricCar = hasPlayerElectricCar(vehicle)
				local isSupercharger = pumpHash == Config.SuperchargerModel
				local isAllowedToFuel = (isElectricCar and isSupercharger) or (not isElectricCar and not isSupercharger)

				if isAllowedToFuel and DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 2.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								if pumpHash == Config.SuperchargerModel then
									DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.EToRefuelEle)
								else
				...
				elseif isNearPump and pumpHash ~= Config.SuperchargerModel then
					local stringCoords = GetEntityCoords(isNearPump)
