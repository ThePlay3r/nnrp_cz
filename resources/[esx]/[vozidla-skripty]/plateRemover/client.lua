...
RegisterCommand("removeplate", function()
    ...
    if not LicencePlate.Index and not LicencePlate.Number then
        ...
        if Distance < 3.5 and not IsPedInAnyVehicle(PlayerPed, false) then
            ...
            Animation()
			SendNUIMessage({type = "ui",display = true,time = 6000,text = "Sundavas SPZ"}) --PROGRESSBAR
			...
			TriggerEvent('esx:showNotification', '~r~ Musis byt u vozidla.')
        end
    else
        -- Notification
		-- exports["mythic_notify"]:SendAlert("error", "You already have a licence plate on you.") Mythic_Notification
		TriggerEvent('esx:showNotification', '~r~ Nemas u sebe potrebnou spz')
    end
end)
    ...
    if LicencePlate.Index and LicencePlate.Number then
		...
		if (Vehicle == LastVehicle) then
			--Cleans variable
				...
				-- Notification and animation
				Animation()
				SendNUIMessage({type = "ui",display = true,time = 6000,text = "Nasazujes SPZ"}) --PROGRESSBAR
				StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
			-- Wait 6 seconds
			Citizen.Wait(6000)
			...
			TriggerEvent('esx:showNotification', '~r~ Tato SPZ nepatri na tohle auto')
		end
        else
            -- Notification
			-- exports["mythic_notify"]:SendAlert("error", "No vehicle nearby.") Mythic_Notification
			TriggerEvent('esx:showNotification', '~r~ Zadne vozidlo pobliz.')
        end
    else
        -- Notification
		-- exports["mythic_notify"]:SendAlert("error", "You already have a licence plate on you.") Mythic_Notification
		TriggerEvent('esx:showNotification', '~r~ Nemas zadnou SPZ znacku.')
    end
end)
