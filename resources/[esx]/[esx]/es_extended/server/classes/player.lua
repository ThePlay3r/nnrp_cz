function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
	...
	self.canCarryItem = function(itemName, count)
		local itemLimit = ESX.Items[itemName].limit
		local playerCount = self.getInventoryItem(itemName).count

		if itemLimit == -1 then
			return true
		end

		if itemLimit >= (playerCount + count) then
			return true
		end
		return false
	end
