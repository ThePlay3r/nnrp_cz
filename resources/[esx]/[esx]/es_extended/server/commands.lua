	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				exports.nnadmin:sendAdminLogToDiscordWithTime('Admin **' .. GetPlayerName(source) .. '** nastavil job: **' .. args[2] .. '** grade: **' .. args[3] .. '** hraci ' .. xPlayer.name)
			else
...
TriggerEvent('es:addGroupCommand', 'car', 'helper', function(source, args, user)
	TriggerClientEvent('esx:spawnVehicle', source, args[1])
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = _U('spawn_car'), params = {{name = "car", help = _U('spawn_car_param')}}})

TriggerEvent('es:addGroupCommand', 'cardel', 'helper', function(source, args, user)
	TriggerClientEvent('esx:deleteVehicle', source)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = _U('delete_vehicle')})

TriggerEvent('es:addGroupCommand', 'dv', 'helper', function(source, args, user)
	TriggerClientEvent('esx:deleteVehicle', source)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = _U('delete_vehicle')})
...
TriggerEvent('es:addGroupCommand', 'giveitem', 'mod', function(source, args, user)
	...
	if count ~= nil then
		if xPlayer.getInventoryItem(item) ~= nil then
			...
			exports.nnadmin:sendAdminLogToDiscordWithTime('Admin **' .. GetPlayerName(source) .. '** dal item: **' .. item .. '** pocet: **' .. count .. '** hraci ' .. xPlayer.name)
		else
...
TriggerEvent('es:addGroupCommand', 'giveweapon', 'mod', function(source, args, user)
	...
	exports.nnadmin:sendAdminLogToDiscordWithTime('Admin **' .. GetPlayerName(source) .. '** dal zbran: **' .. weaponName .. '** naboje: **' .. args[3] .. '** hraci ' .. xPlayer.name)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
