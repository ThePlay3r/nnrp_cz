...
function setIdentity(identifier, data, callback)
	MySQL.Async.execute('UPDATE `users` SET `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height, `lore` = @lore WHERE identifier = @identifier', {
		['@identifier']		= identifier,
...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			...
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]^0', 'Failed to set character, try again later or contact the server admin!' } })
		end
	end)
end)

RegisterServerEvent('esx_identity:setLore')
AddEventHandler('esx_identity:setLore', function(lore)
	local _source = source
	local hex = GetPlayerIdentifiers(_source)[1]
	MySQL.Async.execute('UPDATE `users` SET `lore` = @lore WHERE identifier = @identifier', {
		['@identifier']	= hex,
		['@lore'] = lore,
	}, function(rowsChanged)
		TriggerClientEvent('chat:addMessage', _source, { args = { '^1[LORE]^0', 'Lore ulozeno.' } })
	end)
end)
...
TriggerEvent('es:addGroupCommand', 'lore', 'user', function(source, args, user)
	...
	if xPlayer ~= nil then
		...
		function(users)
			...
			if user.lore == nil then
				TriggerClientEvent('esx_identity:showRegisterLore', source, user)
			else
				TriggerClientEvent(
					'chat:addMessage',
					source,
					{ args = { '^1[LORE]: ', user.lore } })
			end
		end
		)
	end
end, {help = "Zobrazit lore postavy (viditelne pouze pro tebe)"})
		...
		function(users)
			...
			if user.skin == nil then
				...
				TriggerClientEvent(
					'chat:addMessage',
					source,
					{ args = { '^1[POSTAVA]', ' Nemuzes si zalozit dalsi postavu!' } })
			end
		end
		)
	end
end, {help = "Zaregistrovat novou postavu"})

TriggerEvent('es:addGroupCommand', 'char', 'helper', function(source, args, user)
	getIdentity(source, function(data)
		if data.firstname == '' then
...
TriggerEvent('es:addGroupCommand', 'charlist', 'helper', function(source, args, user)
...
TriggerEvent('es:addGroupCommand', 'charselect', 'helper', function(source, args, user)
...
TriggerEvent('es:addGroupCommand', 'chardel', 'helper', function(source, args, user)
	local charNumber = tonumber(args[1])
