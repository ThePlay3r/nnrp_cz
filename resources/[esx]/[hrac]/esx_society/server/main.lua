...
RegisterServerEvent('esx_society:putVehicleILovePizzaInGarage')
AddEventHandler('esx_society:putVehicleILovePizzaInGarage', function(societyName, vehicle)
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	local secondSociety = nil

	if SecondSocieties[society] then
		secondSociety = SecondSocieties[society]
	end

	MySQL.Async.fetchAll('SELECT name, firstname, lastname, identifier, job, job_grade FROM users WHERE job = @job OR job = @job2 ORDER BY job_grade DESC', {
		['@job'] = society,
		['@job2'] = secondSociety
	}, function (results)
		local employees = {}
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'lowerboss1' or xPlayer.job.grade_name == 'lowerboss2' or xPlayer.job.grade_name == 'lowerboss3'
...
ESX.RegisterServerCallback('esx_society:setJobSalary', function(source, cb, job, grade, salary)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'lowerboss1' or xPlayer.job.grade_name == 'lowerboss2' or xPlayer.job.grade_name == 'lowerboss3' then
...
ESX.RegisterServerCallback('esx_society:getOnlILovePizzainePlayers', function(source, cb)
	local xPlayers = ESX.GetPlayers()
	local players  = {}
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				TriggerClientEvent('esx:showNotification', xTarget.source, _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)))
			end
