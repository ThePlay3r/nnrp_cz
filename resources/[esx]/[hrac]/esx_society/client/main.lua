...
function OpenEmployeeList(society)
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		local elements = {
			head = {'OFF Duty', _U('employee'), 'OOC Nick', _U('grade'), _U('actions')},
			rows = {}
		}

		if SecondSocieties[society] == nil then
			elements.head = {_U('employee'), 'OOC Nick', _U('grade'), _U('actions')}
		end

		for i=1, #employees, 1 do
			local offJob = SecondSocieties[society]
			local gradeLabel = (employees[i].job.grade_label == '' and employees[i].job.label or employees[i].job.grade_label)
			local rowLabel = nil

			if offJob == employees[i].job.name then
				rowLabel = 'OFF'
			end
...
function OpenRecruitMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:getOnlILovePizzainePlayers', function(players)
		...
		for i=1, #players, 1 do
			local jobName = players[i].job.name
			-- local hasAllowedJob = jobName == 'unemployed' or jobName == 'fueler' or jobName == 'miner' or jobName == 'poolcleaner' or jobName == 'zahradnik'
			local hasAllowedJob = jobName ~= society
