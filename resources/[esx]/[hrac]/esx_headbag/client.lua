...
if closestPlayer == -1 or closestDistance > 2.0 then 
    ESX.ShowNotification('~r~Zadny hrac neni v okoli')
else
  if not HaveBagOnHead then
    TriggerServerEvent('esx_worek:sendclosest', GetPlayerServerId(closestPlayer))
    ESX.ShowNotification('~g~Nasadil jsi pytel na hlavu ~w~' .. GetPlayerName(closestPlayer))
    TriggerServerEvent('esx_worek:closest')
  else
    ESX.ShowNotification('~r~Tento hrac uz ma pytel na hlave')
  end
end
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~Nekdo ti sundal pytel z hlavy!')
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Nasadit pytel hraci na hlavu', value = 'puton'},
          {label = 'Sundat pytel hraci z hlavy', value = 'putoff'},
