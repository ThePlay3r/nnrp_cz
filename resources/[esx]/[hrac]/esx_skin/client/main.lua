...
local firstSpawn, zoomOffset, camOffset, heading = true, 0.0, 0.0, 90.
...
function OpenMenu(submitCb, cancelCb, restrict)
    ...
    ESX.ShowNotification('Otoceni kamery ~g~4~s~ a ~g~6~s~. Otoceni postavy pouzit ~g~A~s~ a ~g~D~s~')
...
Citizen.CreateThread(function()
    while true do
        ...
        if isCameraActive then
            ...
            SetCamCoord(cam, pos.x, pos.y, coords.z - 0.1)
            PointCamAtCoord(cam, posToLook.x, posToLook.y, coords.z - 0.1)
        else
            Citizen.Wait(500)
        end
    end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isCameraActive then
            ...
            if IsControlPressed(0, 234) then
                playerHeading = playerHeading - 1.0
...
--                 print(ESX.DumpTable(skin), skin, "spawn")
--                 if skin == nil then
