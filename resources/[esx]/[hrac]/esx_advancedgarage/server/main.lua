...
-- Fetch Owned Cars
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedCars', function(source, cb, owner, type)
	local ownedCars = {}
	local xPlayer = ESX.GetPlayerFromId(source)
	local ownerIdentif = xPlayer.identifier
	local vehType = 'civ'
...
-- Store Vehicles
ESX.RegisterServerCallback('esx_advancedgarage:storeVehicle', function (source, cb, vehicleProps, secondOwner)
	local ownedCars = {}
	local vehplate = vehicleProps.plate:match("^%s*(.-)%s*$")
	local vehiclemodel = vehicleProps.model
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE (owner = @owner OR owner = @secondOwner) AND @plate = plate', {
		['@owner'] = xPlayer.identifier,
		...
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				MySQL.Async.execute('UPDATE owned_vehicles SET vehicle = @vehicle WHERE (owner = @owner OR owner = @secondOwner) AND plate = @plate', {
					['@owner'] = xPlayer.identifier,
