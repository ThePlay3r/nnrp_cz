...
function OpenMenuGarage(PointType)
	...
	if PointType == 'car_garage_point' then
		table.insert(elements, {label = 'Auta', type = 'cars', value = 'list_owned_cars'})
		table.insert(elements, {label = 'Specialky', type = 'exclusive', value = 'list_owned_cars'})
		table.insert(elements, {label = 'Frakcni auta', type = 'fraction', value = 'list_owned_cars'})
		table.insert(elements, {label = 'Motorky', type = 'motorky', value = 'list_owned_cars'})
		table.insert(elements, {label = 'Kola', type = 'kola', value = 'list_owned_cars'})
		table.insert(elements, {label = 'Vozidla Los Santos', type = 'autalossantos', value = 'list_owned_cars'})
		table.insert(elements, {label = 'Vsechna vozidla', type = 'all', value = 'list_owned_cars'})
		if ESX.PlayerData.job.name == this_Garage.specialJob and this_Garage.specialJob == 'bazar' then
			table.insert(elements, {label = 'Bazar auta', owner = 'autobazar', type = 'all', value = 'list_owned_cars'})
		end
		if ESX.PlayerData.job.name == this_Garage.specialJob and this_Garage.specialJob == 'benefactor' then
			table.insert(elements, {label = 'Bazar auta', owner = 'benefactor', type = 'all', value = 'list_owned_cars'})
		end
	...
	elseif PointType == 'car_store_point' then
		local storePoint = {label = _U('store_owned_cars'), owner = 'autobazar', job = 'bazar', value = 'store_owned_cars'}
		if this_Garage.specialJob == 'benefactor' then
			storePoint.owner = 'benefactor'
			storePoint.job = 'benefactor'
		end
		table.insert(elements, storePoint)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'garage_menu', {
		...
		if action == 'list_owned_cars' then
			ListOwnedCarsMenu(data.current.type, data.current.owner, data.current.job)
		...
		elseif action== 'store_owned_cars' then
			StoreOwnedCarsMenu(data.current.owner, data.current.job)
...
function ListOwnedCarsMenu(type, owner)
	local elements = {}
	local jobVeh = nil
	if type == 'fraction' then
		jobVeh = ESX.PlayerData.job.name
	end
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
			...
			ESX.TriggerServerCallback('esx_vehicleshop:getVehicles', function(vehicles)
				local vehWithCategories = {}
				for _,v in ipairs(vehicles) do
					vehWithCategories[GetHashKey(v.model)] = v.category
				end

				for _,v in pairs(ownedCars) do
					local hashVehicule = v.vehicle.model
					local aheadVehName = GetDisplayNameFromVehicleModel(hashVehicule)
					local vehicleName  = exports.nnvozidla:getVehicleName(aheadVehName)
					local vehicleCategory = vehWithCategories[hashVehicule]
					local plate        = v.plate
					local labelvehicle

					if Config.ShowVehicleLocation then
						local state = '<span style="color: rgb(0, 207, 10);">K dispozici</span>'
						if not v.stored then
							state = '<span style="color: rgb(181, 0, 0);">Na odtahovce</span>'
						end
						local plate = '<span style="color: rgb(173, 211, 255); font-weight:bold;">' .. plate .. '</span>'
						labelvehicle = vehicleName .. ' | ' .. plate .. ' | <b>' .. state .. '</b>'
					else
						if v.stored then
					...
					if type == 'exclusive' then
						if vehicleCategory == type then
							table.insert(elements, {label = labelvehicle, value = v, vehicleName = vehicleName})
						end
					elseif type == 'motorky' then
						if vehicleCategory == type then
							table.insert(elements, {label = labelvehicle, value = v, vehicleName = vehicleName})
						end
					...
					elseif type == 'autalossantos' then
						if vehicleCategory == type or vehicleCategory == 'vip' then
							table.insert(elements, {label = labelvehicle, value = v, vehicleName = vehicleName})
						end
					elseif type == 'cars' then
						if
							vehicleCategory ~= 'exclusive' and vehicleCategory ~= 'motorky' and vehicleCategory ~= 'kola' and vehicleCategory ~= 'autalossantos' and vehicleCategory ~= 'vip'
						then
							table.insert(elements, {label = labelvehicle, value = v, vehicleName = vehicleName})
						end
					else
				...
				table.sort(elements, function(a, b)
					if not a.vehicleName then
						return 1
					end
					if not b.vehicleName then
						return -1
					end
...
function StoreOwnedCarsMenu(owner, job)
...
function SpawnVehicle(vehicle, plate)
	ESX.Game.SpawnVehicle(vehicle.model, {
		...
		WashDecalsFromVehicle(callback_vehicle, 1.0)
