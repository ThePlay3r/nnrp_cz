...
function OpenPropertyMenu(property)
  ...
  if PropertyIsOwned(property) then
    ...
    if not Config.EnablePlayerManagement then
      ...
      table.insert(elements, {label = _U('rent', property.price / Config.RentKoeficient),   value = 'rent'})
    end
    ...
    function(data, menu)
      ...
      ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gateway_available_properties_actions',
        {
          ...
          align    = 'top-left',
          elements = {
            {label = _U('buy', data.current.price), value = 'buy'},
            {label = _U('rent', data.current.price / Config.RentKoeficient),   value = 'rent'},
