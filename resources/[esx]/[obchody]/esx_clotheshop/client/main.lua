						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing', {
							...
							if data2.current.value == 'yes' then
								ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
									...
									menu3.close()
									local outfitLabel = data3.value
									TriggerEvent('skinchanger:getSkin', function(skin)
										ESX.TriggerServerCallback('nncore:saveNewOutfit', function(success)
											if success then
												ESX.ShowNotification(_U('saved_outfit'))
											else
												ESX.ShowNotification('Outfit se nepodarilo ulozit, uz existuje outfit se stejnym nazvem nebo jsi dosahl/a limitu 25 obleceni')
											end
										end, outfitLabel, skin)
									end)
									-- TriggerEvent('skinchanger:getSkin', function(skin)
...
Citizen.CreateThread(function()
	for i=1, #Config.Shops, 1 do
		...
		if Config.Shops[i].hideBlip ~= true then
