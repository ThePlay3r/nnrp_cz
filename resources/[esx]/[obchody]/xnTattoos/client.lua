...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Odstranil/a jsi tetovani ~y~" .. GetLabelText(label) .. "~s~")
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		SetTextComponentFormat('STRING')
		AddTextComponentString("Stiskni ~INPUT_CONTEXT~ pro otevreni katalogu tetovani")
		DisplayHelpTextFromStringLabel(0, 0, 1, -1)
		-- scaleType = setupScaleform("instructional_buttons", "Otevrit Katalog tetovani", 38)
		-- scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Zmenit pohled kamery", 21, "Zmenit pruhlednost", {90, 89}, "Koupit/Odstranit tetovani", 191)
		scaleString = sType
	end
end
...
Citizen.CreateThread(function()
    ...
    JayMenu.SetSubTitle('tattoo', "Casti tela")
