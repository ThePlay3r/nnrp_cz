...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "Zakoupil/a jsi tetovani ~y~" .. tattooName .. "~s~ za ~g~$" .. price)
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "Nemas dostatek penez")
		cb(false)
	end
end)
