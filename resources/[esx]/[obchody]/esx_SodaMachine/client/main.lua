...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				ESX.ShowNotification('Prodávaní... Prosím cekejte')
				...
				Citizen.Wait(700)
				ESX.ShowNotification('Koupil jsi plechovku ~r~Coly~s~ za ~g~$10~s~. Dekujeme za váš nákup!')
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Tento automat je docasne mimo provoz')
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Automat je prázdný')
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification('Vyskytl se problém se strojem a vzal vám peníze')
			FreezeEntityPosition(ped, false)
			end)
		end
end)