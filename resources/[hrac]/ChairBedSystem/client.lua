				...
				if GetDistanceBetweenCoords(oEntityCoords.x, oEntityCoords.y, oEntityCoords.z,pedPos) < 15.0 then
					if oSelectedObject ~= 0 then
						...
						if oSelectedObject ~= objects.object then
							...
							objects.CustomDirections = v.customDirections
							objects.isBed = v.bed
						end
					end
				end
...
CreateThread(function()
	while true do
		...
		if objects.object ~= nil and objects.ObjectVertX ~= nil and objects.ObjectVertY ~= nil and objects.ObjectVertZ ~= nil and objects.OjbectDir ~= nil and objects.isBed ~= nil then
			...
			if GetDistanceBetweenCoords(objectcoords.x, objectcoords.y, objectcoords.z,getPlayerCoords) < 1 and not using then
				if objects.isBed == true then
					DrawText3Ds(objectcoords.x, objectcoords.y, objectcoords.z+0.30, "Stiskni ~g~E~w~ k lehnuti na "..anim)
					DrawText3Ds(objectcoords.x, objectcoords.y, objectcoords.z+0.20, "~w~ Pomoci ~g~sipek~w~ zmen pozici")
					...
					if IsControlJustPressed(0, 38) then
						PlayAnimOnPlayer(objects.object,objects.ObjectVertX,objects.ObjectVertY,objects.ObjectVertZ,objects.OjbectDir, objects.CustomDirections, objects.isBed, player, objectcoords)
					end
				else
					DrawText3Ds(objectcoords.x, objectcoords.y, objectcoords.z+0.30, " ~g~E~w~ pro sednuti")
			...
			if using == true then
				Draw2DText("Stiskni ~g~F~w~ aby ses zvednul!",0,1,0.5,0.92,0.6,255,255,255,255)
...
function PlayAnimOnPlayer(object,vertx,verty,vertz,dir, CustomDirections, isBed, ped, objectcoords)
	...
	using = true
	if isBed == false then
		if CustomDirections and CustomDirections[objectcoords] then
			TaskStartScenarioAtPosition(ped, Config.objects.SitAnimation, objectcoords.x+vertx, objectcoords.y-verty, objectcoords.z-vertz, CustomDirections[objectcoords].direction, 0, true, true)
		else
