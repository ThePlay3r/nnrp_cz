...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	...
	if (self.plugin_data.radioChannel ~= -1 and self.myChannels[self.plugin_data.radioChannel]) then
		if (string.match(self.myChannels[self.plugin_data.radioChannel].name, "Call")) then
			...
			info = info  .. "<br> [Vysilacka] " .. self.myChannels[self.plugin_data.radioChannel].name;
		end
	end
...
function TokoVoip.initialize(self)
	...
	Citizen.CreateThread(function()
		while (true) do
			...
			if (isRadioSpeaking and self.plugin_data.radioChannel ~= -1 and self.config.radioEnabled) then -- Talk on radio
...
RegisterCommand("+vysilackaMluvit", function(source, args, rawCommand)
	isRadioSpeaking = true
end)
RegisterCommand("-vysilackaMluvit", function(source, args, rawCommand)
	isRadioSpeaking = false
end)
RegisterKeyMapping('+vysilackaMluvit', 'Mluvit do vysilacky/telefonu', 'keyboard', 'u')
