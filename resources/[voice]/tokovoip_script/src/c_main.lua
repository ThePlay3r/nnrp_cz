...
RegisterNetEvent("tokovoip:setSpectating");
AddEventHandler("tokovoip:setSpectating", function(playerServerId, isSpectating)
	Spectating[playerServerId] = isSpectating
end)
...
local function clientProcessing()
	...
	local isMeSpectating = Spectating[GetPlayerServerId(PlayerId())]
	...
	for i=1, #playerList do
		...
		do
			...
			if isMeSpectating then	
				playerPos = vector3(playerPos.x, playerPos.y, playerPos.z + 35)
			end
			local dist = #(localPos - playerPos);
			local isSpectating = Spectating[playerServerId]
...
RegisterCommand('radiovolume', function(source, args)
	setRadioVolume(tonumber(args[1]))
end)
